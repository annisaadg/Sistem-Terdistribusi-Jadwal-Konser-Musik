import random
from mqtt_common import connect_mqtt  # Import reusable function

# Kelas untuk mengirim pesan ke topik "YG Entertainment"
class YGPublisher:
    def __init__(self):
        self.topic = "YG Entertainment" # Topik untuk mengirim pesan
        self.client_id = f'python-mqtt-{random.randint(0, 1000)}' # Menghasilkan ID klien acak
        self.client = connect_mqtt(self.client_id, "YG Entertainment") # Menghubungkan ke broker MQTT dengan ID klien dan nama agensi

    def publish(self, pesan, jadwal):
        msg = f"{pesan} jadwal:{jadwal}" # Memformat pesan dengan jadwal
